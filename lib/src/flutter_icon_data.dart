...
class FlutterIconData extends IconData {
  ...
  const FlutterIconData.antDesignFilled(int codePoint)
      : this(codePoint, 'AntDesign-Filled');

  const FlutterIconData.antDesignOutlined(int codePoint)
      : this(codePoint, 'AntDesign-Outlined');

  const FlutterIconData.brandico(int codePoint) : this(codePoint, 'Brandico');

  const FlutterIconData.elusiveIcons(int codePoint)
      : this(codePoint, 'Elusive-Icons');
  ...
  const FlutterIconData.fontAwesome5Regular(int codePoint)
      : this(codePoint, 'FontAwesome5_Regular');
  ...
  const FlutterIconData.linearicons(int codePoint)
      : this(codePoint, 'Linearicons-Free');
  ...
  const FlutterIconData.materialIconsBaseline(int codePoint)
      : this(codePoint, 'MaterialIcons-Baseline');

  const FlutterIconData.materialIconsOutline(int codePoint)
      : this(codePoint, 'MaterialIcons-Outline');

  const FlutterIconData.materialIconsRound(int codePoint)
      : this(codePoint, 'MaterialIcons-Round');

  const FlutterIconData.materialIconsSharp(int codePoint)
      : this(codePoint, 'MaterialIcons-Sharp');
  ...
  const FlutterIconData.webSymbols(int codePoint)
...
class IconDataDuotone extends IconData {
  ...
  const IconDataDuotone(int codePoint, {this.secondary})
      : super(codePoint, fontFamily: 'MaterialIcons-TwoTone', fontPackage: 'flutter_icons');
}
